        ...
        for i, split in enumerate(splits):
            ...
            with open(split, 'r') as f:
                while True:
                    ...
                    if line:
                        self.imgs.append(os.path.join(root, "img/", line+".jpg"))
                        if self.mode == 'train' or self.mode == 'val' or self.mode == 'val-rawsize':
                            self.labels.append(os.path.join(root, "anno/", line+".png"))
                    else:
                        break
        # print(self.imgs)
    ...
    def __init__(self, root, split, w=512, h=512, mode="train", resize=True, crop=False):
        ...
        if self.mode == 'train' or self.mode == 'semi-train':
            self.albu = albu.Compose([
                ...
                # albu.RandomCrop(self.h, self.w, p=1),
                albu.RandomResizedCrop(512, 512, scale=(0.8, 1.0), ratio=(0.75, 1.3333333333333333), p=1),
                albu.Resize(self.h, self.w, p=1),
            ])
        elif self.mode == 'val':
            self.albu = albu.Compose([
...
if __name__ == "__main__":
    ...
    trainset = ManiDatasetAug([root], split=["train.txt"], w=512, h=512, )
    trainloader = data.DataLoader(trainset, batch_size=1, shuffle=True)
