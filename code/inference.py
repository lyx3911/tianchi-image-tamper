...
def TTA_val(model, valloader, th=0.5):
    losses = AverageMeter()
    scores = AverageMeter()
    ious = AverageMeter()
    model.eval()
...
def TTA_inference_single(model, img, th=0.3, alpha=0.3):
    ...
    # fake_seg = 255.0 * fake_seg
    fake_seg = fake_seg.astype(np.uint8)

    # print(fake_seg.shape)
    return fake_seg, max_score

def TTA_inference_two(model, model1, img, th=0.3, alpha=0.3):
    # model slidewindow, model1 resize
    transform_pil = transforms.Compose([
        transforms.ToPILImage(),
    ])
    img = img.cuda().view(-1, img.shape[0], img.shape[1], img.shape[2])
...
if __name__ == '__main__':
    ...
    # init dataset
    testset = ManiDataset([args.root], ['test.txt'], mode='test', resize=False)
    
    for img, img_name, fake_size in tqdm(testset):
        # print(img.shape, img_name, fake_size)
        seg, max_score = TTA_inference_single(model, img, th=args.th, alpha=0.3)
        # seg, max_score = TTA_inference_two(model, model1, img, th=args.th, alpha=0.35)
        # np.save("{}/{}.npy".format(args.save_dir, img_name), seg)
        seg = cv2.resize(seg, (fake_size[1], fake_size[0]))
